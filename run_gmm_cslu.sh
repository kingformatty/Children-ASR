...
wav_dir=/media/kingformatty/king/SLT2021_CSRC_original/SLT2021_CSRC_DATA/
...
if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
  for part in SetC_train; do #SetA_dev SetC1_dev SetC2_dev; do
...
for train in SetC_train; do  
  utils/subset_data_dir.sh $datadir/$train 10000 $datadir/${train}_30k
  utils/subset_data_dir.sh $datadir/$train 20000 $datadir/${train}_90k
done
fi
...
for train in SetC_train; do
  ...
  steps/train_mono.sh --nj 8 --cmd "$train_cmd" \
    $datadir/${train}_30k $datadir/lang $expdir/mono_${train}

  # decode using the monophone model
  utils/mkgraph.sh $datadir/lang_test $expdir/mono_${train} $expdir/mono_${train}/graph
...
for train in SetC_train; do
  for test in SetC_dev; do
    steps/decode_fmllr.sh --nj 2 --cmd "$decode_cmd" \
      $expdir/tri4b_${train}/graph $datadir/$test $expdir/tri4b_${train}/decode_$test
...
./local/chain/run_tdnnf.sh --train_set $train \
	--test_sets $dev --gmm tri4b_SetC_train 
fi
